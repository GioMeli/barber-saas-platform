        ...
        <header className="sticky top-0 z-30 border-b border-border/70 bg-background/92 backdrop-blur-xl">
          <div className="flex min-h-16 min-w-0 items-center gap-2 px-3 py-2 sm:min-h-[72px] sm:px-5 lg:px-7">
            ...
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-extrabold sm:text-base">{current.key === 'training' ? t('demoChrome.courses') : current.key === 'home' ? t('demoChrome.home') : t(current.labelKey)}</div>
              <div className="hidden truncate text-xs text-muted-foreground sm:block">{scenario.businessName} · {t('demoChrome.ownerDemo')}</div>
            </div>
            <div className="hidden max-w-[290px] flex-1 md:block"><div className="flex h-10 items-center rounded-xl border bg-card px-3 text-xs text-muted-foreground"><Search className="mr-2 h-4 w-4" />{t('demoChrome.search')}<span className="ml-auto rounded-md border bg-muted px-1.5 py-0.5 text-[10px]">Ctrl K</span></div></div>
            <Button type="button" size="icon" className="h-10 w-10 rounded-xl" onClick={() => navigate('/demo/calendar?action=new')} aria-label={t('demoChrome.addAppointment')}><Plus className="h-4 w-4" /></Button>
            <Button type="button" variant="outline" size="icon" className="h-10 w-10 rounded-xl" aria-label={t('demoChrome.notifications')}><Bell className="h-4 w-4" /></Button>
          ...
          <div className="flex flex-col gap-2 border-t border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-900 sm:flex-row sm:items-center sm:justify-between sm:px-5 lg:px-7">
            <div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 shrink-0 text-emerald-700" /><span><strong>{t('demoChrome.modeTitle')}</strong> {t('demoChrome.modeText')}</span></div>
            <div className="flex shrink-0 gap-2"><Button type="button" variant="ghost" size="sm" className="h-8 rounded-lg text-emerald-800 hover:bg-emerald-100" onClick={reset}><RefreshCw className="mr-1.5 h-3.5 w-3.5" />{t('demoChrome.reset')}</Button><Button asChild variant="ghost" size="sm" className="h-8 rounded-lg text-emerald-800 hover:bg-emerald-100"><Link to="/"><ArrowLeft className="mr-1.5 h-3.5 w-3.5" />{t('demoChrome.exit')}</Link></Button></div>
          </div>
        </header>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-sidebar text-sidebar-foreground">
      <div className="border-b border-sidebar-border px-4 py-4">
        <div className="flex items-center gap-3"><img src="/brand/velliqo-mark-transparent-v2.png" alt="Velliqo" className="h-11 w-11 object-contain" /><div><div className="text-base font-extrabold text-white">Velliqo</div><div className="text-[10px] font-semibold uppercase tracking-[.18em] text-sidebar-foreground/50">{t('demoChrome.ownerWorkspace')}</div></div>{mobile && <button type="button" className="ml-auto text-white/60" onClick={onNavigate}><X className="h-5 w-5" /></button>}</div>
        <div className="mt-4 rounded-2xl border border-white/[.07] bg-white/[.045] p-3"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-sidebar-primary font-bold text-sidebar-primary-foreground">{scenario.businessName.charAt(0)}</div><div className="min-w-0"><div className="truncate text-sm font-bold text-white">{scenario.businessName}</div><div className="truncate text-[11px] text-sidebar-foreground/55">{t('demoChrome.sampleOwner')}</div></div></div><select value={scenarioIndex} onChange={(event) => switchScenario(Number(event.target.value))} className="mt-3 h-9 w-full rounded-xl border border-white/10 bg-white/[.06] px-2 text-xs text-white outline-none"><option value={0} className="text-slate-900">{t('demoChrome.physio')}</option><option value={1} className="text-slate-900">{t('demoChrome.pet')}</option><option value={2} className="text-slate-900">{t('demoChrome.car')}</option></select></div>
      </div>
      <div className="scrollbar-subtle min-h-0 flex-1 overflow-y-auto px-3 py-4"><div className="mb-3 px-1 text-[10px] font-bold uppercase tracking-[.18em] text-sidebar-foreground/40">{t('demoChrome.workspaceNavigation')}</div><nav className="grid grid-cols-2 gap-2">{DEMO_NAVIGATION_ITEMS.map((item) => { const active = isDemoNavigationItemActive(location.pathname, item.path); return <Link key={item.key} to={item.path} onClick={onNavigate} className={cn('group relative flex min-h-[74px] min-w-0 flex-col items-center justify-center gap-2 rounded-2xl border px-2 py-3 text-center transition', active ? 'border-sidebar-primary/35 bg-sidebar-primary text-sidebar-primary-foreground shadow-[0_12px_30px_hsl(var(--sidebar-primary)/0.22)]' : 'border-white/[.055] bg-white/[.025] text-sidebar-foreground/68 hover:bg-sidebar-accent hover:text-white')}><item.icon className="h-5 w-5" /><span className="line-clamp-2 text-[11px] font-semibold leading-4">{item.key === 'training' ? t('demoChrome.courses') : item.key === 'home' ? t('demoChrome.home') : t(item.labelKey)}</span>{item.key === 'ai' && <span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-fuchsia-300" />}</Link>; })}</nav></div>
      <div className={cn('border-t border-sidebar-border p-3', mobile && 'safe-bottom')}><Button asChild variant="ghost" className="w-full justify-start rounded-xl text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-white"><Link to="/"><ArrowLeft className="mr-2 h-4 w-4" />{t('demoChrome.exit')}</Link></Button></div>
    </div>
  );
}
