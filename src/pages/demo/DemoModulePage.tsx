...
const moduleLabels: Record<string, { title: string; description: string }> = {
  ...
  settings: { title: 'Settings', description: 'Try owner preferences without changing any production configuration.' },
};
...
function DemoModule({ moduleKey }: { moduleKey: string }) {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6">
      ...
      {moduleKey === 'settings' && <SettingsModule />}
...
function MarketingModule() { const d=useDemoOwner(); return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]"><Panel title="Campaigns" subtitle="Sending is disabled in demo mode"><div className="grid gap-3">{d.campaigns.map((campaign)=><div key={campaign.id} className="flex items-center justify-between rounded-xl border p-4"><div><div className="font-extrabold">{campaign.name}</div><div className="text-xs text-muted-foreground">Audience: {campaign.audience} customers</div></div><span className={cn('rounded-full px-2.5 py-1 text-[10px] font-bold uppercase',campaign.status==='draft'?'bg-amber-100 text-amber-800':'bg-violet-100 text-violet-700')}>{campaign.status}</span></div>)}</div></Panel><Panel title="Delivery safety" subtitle="Production boundaries"><ul className="space-y-3 text-sm text-muted-foreground">{['Consent is checked before delivery','Campaigns require review and confirmation','No email or SMS provider is called in demo mode'].map((item)=><li key={item} className="flex gap-2"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />{item}</li>)}</ul></Panel></div>; }
...
function BillingModule() { return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]"><Panel title="Velliqo Professional" subtitle="Demo subscription"><div className="rounded-2xl border border-violet-200 bg-violet-50 p-5"><div className="flex items-center justify-between"><div><div className="text-xs font-bold uppercase tracking-[.16em] text-violet-700">Current plan</div><h2 className="mt-2 text-2xl font-extrabold text-violet-950">Professional</h2></div><CreditCard className="h-8 w-8 text-violet-600"/></div><div className="mt-5 text-3xl font-extrabold">€49<span className="text-sm font-semibold text-muted-foreground"> / month</span></div></div></Panel><Panel title="Invoices" subtitle="Example billing history"><div className="space-y-3"><StatusRow label="July 2026" value="Paid - €49"/><StatusRow label="June 2026" value="Paid - €49"/><StatusRow label="May 2026" value="Paid - €49"/></div></Panel></div>; }
...
function SettingsModule() { const d=useDemoOwner(); return <div className="grid gap-5 lg:grid-cols-2"><Panel title="Notifications" subtitle="Try the controls locally"><SettingRow label="Appointment reminders" description="Send reminders before appointments" checked={d.settings.reminders} onChange={()=>d.toggleSetting('reminders')}/><SettingRow label="Daily AI briefing" description="Show the owner executive briefing" checked={d.settings.aiBriefing} onChange={()=>d.toggleSetting('aiBriefing')}/></Panel><Panel title="Booking preferences" subtitle="Demo-only settings"><SettingRow label="Online booking" description="Allow customers to book from the public storefront" checked={d.settings.onlineBooking} onChange={()=>d.toggleSetting('onlineBooking')}/><StatusRow label="Language" value="English"/><StatusRow label="Timezone" value="Europe/Nicosia"/></Panel></div>; }
...
function SettingRow({ label, description, checked, onChange }: { label: string; description: string; checked: boolean; onChange: () => void }) { return <div className="flex items-center justify-between gap-4 border-b py-4 last:border-0"><div><div className="text-sm font-extrabold">{label}</div><div className="mt-1 text-xs text-muted-foreground">{description}</div></div><Switch checked={checked} onCheckedChange={onChange}/></div>; }
