...
export default function Contact() {
  ...
  const businessTypes = t('marketingSite.pages.contact.types', { returnObjects: true }) as string[];
  ...
  const buildMailto = React.useCallback(() => {
    const subject = form.subject.trim() || t('marketingSite.pages.contact.mail.defaultSubject');
    const body = [
      t('marketingSite.pages.contact.mail.heading'),
      '',
      `${t('marketingSite.pages.contact.mail.name')}: ${form.fullName.trim()}`,
      `${t('marketingSite.pages.contact.mail.email')}: ${form.email.trim()}`,
      `${t('marketingSite.pages.contact.mail.business')}: ${form.businessName.trim() || t('marketingSite.pages.contact.mail.notProvided')}`,
      `${t('marketingSite.pages.contact.mail.phone')}: ${form.phone.trim() || t('marketingSite.pages.contact.mail.notProvided')}`,
      `${t('marketingSite.pages.contact.mail.type')}: ${form.businessType || t('marketingSite.pages.contact.mail.notSelected')}`,
      '',
      `${t('marketingSite.pages.contact.mail.message')}:`,
      form.message.trim(),
      '',
      t('marketingSite.pages.contact.mail.footer'),
    ].join('\n');
            ...
            <div className="max-w-xl">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Button asChild size="lg" variant="outline" className="h-12 rounded-xl border-white/15 bg-white/[.04] px-6 text-white hover:bg-white/[.08] hover:text-white">
                  <a href={`tel:${CONTACT_PHONE_HREF}`}>{t('marketingSite.pages.contact.callBtn', { phone: CONTACT_PHONE_DISPLAY })}</a>
                </Button>
              </div>
            </div>

            <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              <ContactCard icon={<Mail className="h-5 w-5" />} label={t('marketingSite.pages.contact.labels.email')} value={CONTACT_EMAIL} href={`mailto:${CONTACT_EMAIL}`} />
              <ContactCard icon={<Phone className="h-5 w-5" />} label={t('marketingSite.pages.contact.labels.phone')} value={CONTACT_PHONE_DISPLAY} href={`tel:${CONTACT_PHONE_HREF}`} />
              <ContactCard icon={<MapPin className="h-5 w-5" />} label={t('marketingSite.pages.contact.labels.region')} value={CONTACT_REGION} />
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto grid max-w-[1280px] gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[.72fr_1.28fr] lg:px-8 lg:py-24">
          <aside className="space-y-5">
            <div className="rounded-[2rem] border border-violet-200 bg-gradient-to-br from-violet-50 to-white p-6 shadow-[0_22px_70px_rgba(76,29,149,.10)] sm:p-7">
              ...
              <h2 className="mt-5 text-2xl font-extrabold tracking-tight">{t('marketingSite.pages.contact.startTitle')}</h2>
              <p className="mt-3 text-sm leading-6 text-slate-600">{t('marketingSite.pages.contact.startText')}</p>
              <div className="mt-6 space-y-4">
                <InfoLine icon={<Clock3 className="h-4 w-4" />} title={t('marketingSite.pages.contact.responseTitle')} text={t('marketingSite.pages.contact.responseText')} />
                <InfoLine icon={<ShieldCheck className="h-4 w-4" />} title={t('marketingSite.pages.contact.controlTitle')} text={t('marketingSite.pages.contact.controlText')} />
              </div>
            </div>

            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_16px_55px_rgba(15,23,42,.055)]">
              <div className="text-xs font-extrabold uppercase tracking-[.18em] text-violet-600">{t('marketingSite.pages.contact.planning')}</div>
              <h3 className="mt-3 text-lg font-extrabold">{t('marketingSite.pages.contact.exploreTitle')}</h3>
              <p className="mt-2 text-sm leading-6 text-slate-600">{t('marketingSite.pages.contact.exploreText')}</p>
              <Button asChild variant="outline" className="mt-5 w-full rounded-xl"><Link to="/experience">{t('marketingSite.pages.contact.exploreBtn')}</Link></Button>
            </div>
          </aside>

          <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_28px_90px_rgba(15,23,42,.09)] sm:p-8">
            <div className="max-w-2xl">
              <div className="text-xs font-extrabold uppercase tracking-[.2em] text-violet-600">{t('marketingSite.pages.contact.formEyebrow')}</div>
              <h2 className="mt-3 text-3xl font-extrabold tracking-[-.04em]">{t('marketingSite.pages.contact.formTitle')}</h2>
              <p className="mt-3 text-sm leading-6 text-slate-600">{t('marketingSite.pages.contact.formText')}</p>
            </div>

            <form className="mt-8 grid gap-5" onSubmit={handleSubmit}>
              <div className="grid gap-5 sm:grid-cols-2">
                <FormField label={t('marketingSite.pages.contact.fields.fullName')} required>
                  <Input value={form.fullName} onChange={updateField('fullName')} placeholder={t('marketingSite.pages.contact.placeholders.fullName')} autoComplete="name" required className="h-12 rounded-xl" />
                </FormField>
                <FormField label={t('marketingSite.pages.contact.labels.email')} required>
                ...
                <FormField label={t('marketingSite.pages.contact.fields.businessName')}>
                  <Input value={form.businessName} onChange={updateField('businessName')} placeholder={t('marketingSite.pages.contact.placeholders.businessName')} autoComplete="organization" className="h-12 rounded-xl" />
                </FormField>
                <FormField label={t('marketingSite.pages.contact.labels.phone')}>
                  <InternationalPhoneInput value={form.phone} onChange={(phone) => setForm((current) => ({ ...current, phone }))} />
                </FormField>
              </div>

              <FormField label={t('marketingSite.pages.contact.fields.businessType')}>
                <select value={form.businessType} onChange={updateField('businessType')} className="flex h-12 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                  <option value="">{t('marketingSite.pages.contact.placeholders.businessType')}</option>
                  {businessTypes.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </FormField>

              <FormField label={t('marketingSite.pages.contact.fields.subject')}>
                <Input value={form.subject} onChange={updateField('subject')} placeholder={t('marketingSite.pages.contact.placeholders.subject')} className="h-12 rounded-xl" />
              </FormField>

              <FormField label={t('marketingSite.pages.contact.fields.message')} required>
                <Textarea value={form.message} onChange={updateField('message')} placeholder={t('marketingSite.pages.contact.placeholders.message')} required className="min-h-[180px] resize-y rounded-xl" />
              </FormField>

              <div className="flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs leading-5 text-slate-500">{t('marketingSite.pages.contact.review')}</p>
                <Button type="submit" size="lg" className="h-12 shrink-0 rounded-xl bg-violet-600 px-6 hover:bg-violet-700">
                  {t('marketingSite.pages.contact.openDraft')} <Send className="ml-2 h-4 w-4" />
                </Button>
              </div>

              {draftOpened && (
                <div role="status" className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">
                  {t('marketingSite.pages.contact.draftOpened', { email: CONTACT_EMAIL })}
                </div>
              )}
            </form>
          </div>
