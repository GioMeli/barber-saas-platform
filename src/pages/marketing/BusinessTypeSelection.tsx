...
export default function BusinessTypeSelection() {
  ...
  const visibleGroups = React.useMemo(() => INDUSTRY_CATEGORIES.map((category) => ({
    category,
    industries: getIndustriesByCategory(category.key).filter((item) => { const localized = `${t(`marketingSite.pages.businessTypes.industries.${item.key}.name`)} ${t(`marketingSite.pages.businessTypes.industries.${item.key}.description`)}`.toLowerCase(); return !normalizedQuery || localized.includes(normalizedQuery); }),
  })).filter((group) => group.industries.length > 0), [normalizedQuery, t]);
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(124,58,237,.08),transparent_30%),linear-gradient(180deg,#faf9ff_0%,#f7f7fc_42%,#ffffff_100%)] text-slate-950">
      <MarketingHeader active="business-types" />
      <main>
        <section className="relative overflow-hidden border-b border-violet-200 bg-[radial-gradient(circle_at_12%_18%,rgba(196,181,253,.35),transparent_24%),radial-gradient(circle_at_88%_8%,rgba(232,121,249,.24),transparent_25%),linear-gradient(135deg,#241044_0%,#4c1d95_48%,#7c3aed_100%)] text-white">
          ...
          <div className="relative mx-auto max-w-6xl px-4 py-16 text-center sm:px-6 lg:py-24">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.2em] text-violet-100 backdrop-blur"><Sparkles className="h-4 w-4" />{t('marketingSite.pages.businessTypes.eyebrow')}</div>
            <h1 className="mx-auto mt-5 max-w-4xl text-4xl font-extrabold tracking-[-0.05em] sm:text-5xl lg:text-6xl">{t('marketingSite.pages.businessTypes.title')}</h1>
            <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-violet-100/85 sm:text-lg">{t('marketingSite.pages.businessTypes.description')}</p>
            ...
            <div className="mx-auto mt-9 max-w-3xl rounded-[2rem] border border-white/20 bg-white/12 p-3 shadow-[0_28px_85px_rgba(15,23,42,.35)] backdrop-blur-xl">
              <div className="relative">
                ...
                <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('marketingSite.pages.businessTypes.search')} className="h-14 rounded-2xl border-white bg-white pl-12 text-base text-slate-950 shadow-lg placeholder:text-slate-400" />
              </div>
            </div>
            <div className="mt-6 flex flex-wrap justify-center gap-x-6 gap-y-3 text-xs font-semibold text-violet-100"><TrustItem text={t('marketingSite.pages.businessTypes.trust.0')} /><TrustItem text={t('marketingSite.pages.businessTypes.trust.1')} /><TrustItem text={t('marketingSite.pages.businessTypes.trust.2')} /></div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-[1440px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-col gap-4 rounded-[1.75rem] border border-slate-200/80 bg-white/90 p-5 shadow-[0_16px_45px_rgba(15,23,42,.06)] backdrop-blur sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-violet-100 text-violet-700"><LayoutGrid className="h-5 w-5" /></span><div><div className="font-extrabold">{t('marketingSite.pages.businessTypes.explore')}</div><p className="mt-1 text-xs text-slate-500">{t('marketingSite.pages.businessTypes.matches', { count: visibleCount })}</p></div></div>
            <div className="scrollbar-subtle flex max-w-full gap-2 overflow-x-auto pb-1 sm:max-w-[65%]">
              {visibleGroups.map(({ category }) => <a key={category.key} href={`#${category.key}`} className="whitespace-nowrap rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 transition hover:border-violet-200 hover:bg-violet-50 hover:text-violet-800">{category.icon} {t(`marketingSite.pages.businessTypes.categories.${category.key}.name`)}</a>)}
            </div>
          </div>
                ...
                <section id={category.key} key={category.key} className={`scroll-mt-28 overflow-hidden rounded-[2.25rem] border p-4 shadow-[0_18px_60px_rgba(15,23,42,.07)] sm:p-6 lg:p-7 ${style.shell}`}>
                  ...
                  <div className="mb-6 flex flex-col gap-4 border-b border-black/[.06] pb-6 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div><h2 className="text-2xl font-extrabold tracking-[-.03em]">{t(`marketingSite.pages.businessTypes.categories.${category.key}.name`)}</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600">{t(`marketingSite.pages.businessTypes.categories.${category.key}.description`)}</p></div>
                    </div>
                    <span className={`w-fit rounded-full border px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[.16em] ${style.count}`}>{t('marketingSite.pages.businessTypes.count', { count: industries.length })}</span>
                  </div>
                    ...
                    {industries.map((item) => (
                      ...
                      >
                        ...
                        <div className="flex flex-1 flex-col p-5">
                          <div className="flex items-start justify-between gap-4"><div><div className="text-[9px] font-extrabold uppercase tracking-[.17em] text-violet-600">{t('marketingSite.pages.businessTypes.tailored')}</div><h3 className="mt-2 text-lg font-extrabold tracking-[-.02em] text-slate-950">{t(`marketingSite.pages.businessTypes.industries.${item.key}.name`)}</h3></div><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-400 transition group-hover:border-violet-200 group-hover:bg-violet-600 group-hover:text-white"><ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" /></span></div>
                          <p className="mt-3 flex-1 text-sm leading-6 text-slate-600">{t(`marketingSite.pages.businessTypes.industries.${item.key}.description`)}</p>
                          <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-violet-700">{t('marketingSite.pages.businessTypes.select')}</span><span className="text-xs font-semibold text-slate-400">{t('marketingSite.pages.businessTypes.guided')}</span></div>
                        </div>
                      </Link>
                    ))}
                  </div>
          ...
          {visibleGroups.length === 0 && (
            <div className="rounded-[2rem] border border-dashed border-violet-300 bg-white p-10 text-center shadow-sm">
              ...
              <h2 className="mt-4 text-xl font-extrabold">{t('marketingSite.pages.businessTypes.noneTitle')}</h2>
              <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-500">{t('marketingSite.pages.businessTypes.noneText')}</p>
              <button type="button" onClick={() => setQuery('')} className="mt-5 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-violet-700">{t('marketingSite.pages.businessTypes.clear')}</button>
            </div>
          )}

          <div className="mt-12 flex items-start gap-3 rounded-[1.75rem] border border-violet-200 bg-white p-6 text-violet-950 shadow-[0_16px_45px_rgba(76,29,149,.08)]"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0"/><div><div className="text-sm font-bold">{t('marketingSite.pages.businessTypes.fallbackTitle')}</div><p className="mt-1 text-xs leading-5 text-violet-900/70">{t('marketingSite.pages.businessTypes.fallbackText')}</p></div></div>
        </section>
      </main>
      <MarketingFooter />
    </div>
