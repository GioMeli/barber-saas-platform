                  ...
                  {services.map((service) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex min-w-0 items-center gap-4 rounded-2xl border p-3 text-left transition sm:p-4 ${
                          selected
                            ? 'border-primary bg-primary/8 shadow-sm ring-1 ring-primary/20'
                            : 'bg-card hover:border-primary/40 hover:shadow-sm'
                        }`}
                      >
                        <ServiceThumbnail src={service.image_url} alt={service.name} className="h-16 w-16 rounded-xl sm:h-20 sm:w-20" />
                  ...
                  <div className="space-y-2 rounded-2xl bg-muted/25 p-3">
                    {selectedServices.map((service) => (
                      <div key={service.id} className="flex items-center gap-3">
                        <ServiceThumbnail src={service.image_url} alt={service.name} className="h-10 w-10 rounded-lg" />
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-sm font-semibold">{service.name}</div>
                          <div className="text-xs text-muted-foreground">{t('publicBooking.durationMinutes', { count: service.duration })}</div>
                        </div>
                        <div className="text-sm font-bold">{currency.format(Number(service.price))}</div>
                      </div>
                    ))}
                  </div>
                )}
